...
from app.model import Movimentacao,Docs, Categoria, Formapagamento, Usuario, Empresa, Conta, Planejamento, Alert
...
def utility_processor():
    def credito():
        ...
        debito_dia = Movimentacao.query.with_entities(func.sum(Movimentacao.valor)).join(Categoria).filter(Movimentacao.data_v == hoje, Movimentacao.empresa_id == session['empresa'], Categoria.status == 1)

        debito_mes = 0.0

        credito_dia = Movimentacao.query.with_entities(func.sum(Movimentacao.valor)).join(Categoria).filter(Movimentacao.data_v == hoje, Movimentacao.empresa_id == session['empresa'], Categoria.status == 0)
